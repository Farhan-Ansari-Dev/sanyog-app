...
export const useAppStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      /**
       * setAuth — called after a successful login.
       * Wipes ALL data from any previous session so the new user
       * starts completely clean. The HomeScreen's useEffect will
       * then re-fetch fresh data for the new user.
       */
      setAuth: (token, user) =>
        set({
          ...
          token,
          user,
          ...emptyUserData,   // ← clears stale previous-user data
        }),

      /**
       * clearAuth — called on explicit logout.
       * Wipes everything: auth + all user-specific content.
       */
      clearAuth: () =>
        set({
          isAuthenticated: false,
          token: null,
          user: null,
          ...emptyUserData,   // ← ensures next login starts fresh
        }),
      ...
      loadCatalog: async () => {
        try {
          ...
          console.warn('Failed to load live catalog, using offline fallback');
      ...
      loadApplications: async () => {
        try {
          ...
          console.warn('Failed to load live applications');
        }
      },
      addApplication: (app) =>
      ...
      unreadCount: 0,
      loadNotifications: () => {
        // Will be wired to backend when notification schema is ready
      },
      markAsRead: (id) =>
        set((state) => {
      ...
      queueOfflineRequest: (req) =>
        set((state) => ({ offlineQueue: [...(state.offlineQueue || []), req] })),
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        // Note: applications/notifications are intentionally NOT persisted
        // They are always fetched fresh from the server on login
      }),
    }
  )
);
